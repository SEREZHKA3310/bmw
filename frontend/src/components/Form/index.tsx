import { Button, Card, Flex, Label } from "@gravity-ui/uikit";
    ...
    <Card className={styles.formCard}>
      <form onSubmit={handleSubmit} className={styles.form}>
        <div className={styles.header}>
          <h2>Загрузка файла оценки</h2>
          {needIndicators && (
            <Label size="m">Всего: {needIndicators.length}</Label>
          )}
        </div>
        <div className={styles.resultsArea}>
          {needIndicators &&
            Object.entries(groupIndicatorsByCompetence(needIndicators)).map(
